...
public:
    ...
    static QString placeSuffix(const QString& filename, Serializer::OutputType outputType);

    /** Verify if the file is a jpg or png file. */
    static bool isImageFile(const QString& filepath);

    /** Verify if the file is a text file based on its filename and contents. */
